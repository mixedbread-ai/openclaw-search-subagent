 ...
 */
export function applySearchSubagentDefaults(config) {
    ...
    return changes.length > 0 ? { config: cfg, changes } : null;
}
 ...
 */
export function stripSearchSubagentConfig(cfg) {
    const changes = [];
    const tools = cfg.tools;
    if (removeFromList(tools?.allow, TOOL_NAME))
        changes.push(`tools.allow: removed ${TOOL_NAME}`);
    if (removeFromList(tools?.alsoAllow, TOOL_NAME))
        changes.push(`tools.alsoAllow: removed ${TOOL_NAME}`);
    if (tools && Array.isArray(tools.alsoAllow) && tools.alsoAllow.length === 0)
        delete tools.alsoAllow;
    const agents = cfg.agents;
    const agentList = agents?.list;
    if (Array.isArray(agentList)) {
        const index = agentList.findIndex((agent) => agent?.id === DEFAULT_SEARCH_AGENT_ID &&
...
export function removeSearchSubagentDefaults(config) {
    const cfg = structuredClone(config);
