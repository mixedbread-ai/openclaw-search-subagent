...
export default definePluginEntry({
    ...
    description: "Registers the Mixedbread Toast-1 model provider and a search_subagent tool " +
    ...
    register(api) {
        ...
        api.registerAutoEnableProbe((ctx) => {
            if (ctx.env?.MIXEDBREAD_API_KEY)
                return "MIXEDBREAD_API_KEY is set";
            const cfg = ctx.config;
            if (cfg?.models?.providers?.[PROVIDER_ID])
                return `models.providers.${PROVIDER_ID} is configured`;
