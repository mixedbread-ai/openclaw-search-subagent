...
import { DEFAULT_MODEL_ID, PROVIDER_ID, TOOL_NAME } from "./config.js";
export { TOOL_NAME };
...
const parameters = Type.Object({
    task: Type.String({
        description: "One specific search goal in natural language, stating the expected output. " +
 ...
 */
const defaultCliAgentRunner = (args, timeoutMs) => new Promise((resolve, reject) => {
    execFile("openclaw", args, { timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024 }, (error, stdout, stderr) => {
        if (error && !stdout) {
...
export function splitModelRef(ref, fallbackProvider) {
...
export function extractCliAgentReply(stdout) {
 ...
 */
export function isSubagentRuntimeUnavailableError(error) {
    if (error?.code === "OPENCLAW_SUBAGENT_RUNTIME_REQUEST_SCOPE")
        return true;
    const message = error instanceof Error ? error.message : String(error);
    return message.includes("only available during a gateway request");
}
function resolveTargetAgentId(ctx, config) {
    const cfg = ctx.getRuntimeConfig?.() ?? ctx.runtimeConfig ?? ctx.config;
    const agents = cfg?.agents?.list ?? [];
    const configured = new Set(agents.map((agent) => agent?.id).filter((id) => typeof id === "string"));
    if (configured.has(config.agentId))
        return config.agentId;
    return ctx.agentId ?? "main";
}
export function registerSearchSubagentTool(api, config, runCliAgent = defaultCliAgentRunner) {
    api.registerTool((ctx) => ({
        ...
        async execute(_toolCallId, rawParams) {
            ...
            if (!task)
                throw new Error("search_subagent requires a non-empty `task`.");
            ...
            try {
                ...
                if (wait.status === "timeout") {
                    throw new Error(`Search subagent timed out after ${config.timeoutSeconds}s (session ${sessionKey}). ` +
                ...
                if (!isSubagentRuntimeUnavailableError(error))
                    throw error;
                if (!config.cliFallback) {
                    throw new Error("The in-process subagent runtime is unavailable here and the CLI fallback is disabled " +
                        "(plugins.entries.search-subagent.config.cliFallback: false). " +
