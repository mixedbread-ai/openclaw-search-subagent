 ...
 */
export function registerSearchSubagentCli(api: OpenClawPluginApi): void {
  api.registerCli(
        ...
        .action(async (opts: { dryRun?: boolean }) => {
          ...
          if (!preview) {
            ...
            await api.runtime.config.mutateConfigFile({
              ...
              mutate(draft) {
                changes = stripSearchSubagentConfig(draft as Parameters<typeof stripSearchSubagentConfig>[0]);
              },
            });
          ...
          if ((config as { models?: { providers?: Record<string, unknown> } }).models?.providers?.[PROVIDER_ID]) {
          ...
          if (leftovers.length > 0) logger.info(`Left in place:\n- ${leftovers.join("\n- ")}`);
          logger.info(
            "Finish with: openclaw plugins uninstall search-subagent && openclaw gateway restart",
          );
        });
    },
    { commands: [PLUGIN_ID] },
